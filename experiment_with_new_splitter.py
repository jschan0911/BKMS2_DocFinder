...
# 로깅 설정
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='new_experiment.log', filemode='a')
...
def split_documents(documents, chunk_size, overlap_size):
    # 줄바꿈을 기준으로 먼저 나누고, 그 이후 청크 크기와 중첩 크기를 적용
...
"""
    try:
        ...
        return response.content, sources, tokens_used
...
def save_results_with_rag(dataset, vector_store, chunk_size, overlap_size, vectordb_size):
    ...
    output_file = f"./results/new_c{chunk_size}_o{overlap_size}_{int(vectordb_size / (1024 * 1024))}MB_dataset.csv"
...
# 실험할 청크 크기
chunk_sizes = list(range(1000, 1200, 100))  # 100부터 1000까지 100 단위로 채움
